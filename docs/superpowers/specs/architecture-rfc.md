...
| **Batch-скрипты Python** | Генерация контента через Claude API, валидация, загрузка в БД |
...
| **Claude API (Anthropic)** | Генерация учебного контента | Только в batch-режиме, не в рантайме приложения |
...
- В рантайме приложения **нет зависимостей** от Claude API или gTTS — весь контент сгенерирован заранее.
...
│  │ (Claude API│    │  (schema +   │    │  is_published=F  │    │
│  │  prompts)  │    │   LLM check) │    └──────────────────┘    │
│  └────────────┘    └──────────────┘                            │
│                                                                 │
│  ┌────────────┐    ┌──────────────┐                            │
...
```
scripts/
├── generate_content.py      # генерирует упражнения через Claude API
├── validate_content.py      # валидирует JSON-схемы + LLM-проверка
...
1. Подготовка промпта
   └── prompts/<type>.txt + unit_context (тема, уровень, целевой словарь)
       └── generate_content.py → Claude API (claude-3-5-sonnet)
...
2. Валидация
   └── validate_content.py
       ...
       └── LLM-ревью: Claude API с validation_prompt
           └── Проверяет: языковую корректность, соответствие A2-уровню,
