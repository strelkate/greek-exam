    ...
    return (
      <div className="true-false">
        ...
        >
          <span style={{ flex: 1 }} />
          <div
            style={{ width: 80, display: 'flex', justifyContent: 'space-between', flexShrink: 0 }}
          >
            <span
              style={{
                width: 28,
        ...
        {shuffledStatements.map((stmt) => {
          ...
          return (
            ...
            >
              <p style={{ flex: 1, fontSize: '0.875rem', lineHeight: 1.5, color: '#fff' }}>
                {stmt.text}
              </p>
              <div
                style={{
              ...
              >
                <button
                  ...
                  style={{
                    width: 28,
                    height: 28,
                    border: `2px solid ${submitted && stmt.correct === true ? 'var(--color-success)' : submitted && answer === true && !stmt.correct ? 'var(--color-error)' : answer === true ? 'var(--color-primary)' : 'var(--color-surface-highest)'}`,
                    borderRadius: 6,
                    background:
                      submitted && stmt.correct === true
                        ? 'var(--color-success)'
                        : submitted && answer === true && !stmt.correct
                          ? 'var(--color-error)'
                          : answer === true
                            ? 'var(--color-primary)'
                            : 'transparent',
                ...
                >
                  {answer === true && (
                    <span
                      style={{ fontSize: 16, fontWeight: 700, color: '#0b0d1a', lineHeight: 1 }}
                    >
                      ✓
                    </span>
                  )}
                ...
                <button
                  ...
                  style={{
                    width: 28,
                    height: 28,
                    border: `2px solid ${submitted && stmt.correct === false ? 'var(--color-success)' : submitted && answer === false && stmt.correct ? 'var(--color-error)' : answer === false ? 'var(--color-primary)' : 'var(--color-surface-highest)'}`,
                    borderRadius: 6,
                    background:
                      submitted && stmt.correct === false
                        ? 'var(--color-success)'
                        : submitted && answer === false && stmt.correct
                          ? 'var(--color-error)'
                          : answer === false
                            ? 'var(--color-primary)'
                            : 'transparent',
